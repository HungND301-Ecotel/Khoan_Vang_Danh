...
const MaterialCostUsed = require("../cost/material-cost-used/material-cost-used.model");
const MaterialBudget = require("../cost/material-budget/material-budget.model");
const OtherMaterialCost = require("../cost/other-material-cost/other-material-cost.model");
const Department = require("../reference/department/department.model");
const { PhaseType } = require("../../config/constants");
...
function processBudgetAndUsedData(
  ...
  // 1. Hop nhat budgetCostDetails - loc theo phase neu co, doc thang tu doc (khong con long trong phases[])
  ...
  // 2. Hop nhat materials thuc hien - loc theo phase neu co
...
async function getMonthGroupedAllScopes(department, fromMonth, toMonth) {
  ...
  const result = months.map((month) => {
    ...
    // otherTasks: OtherMaterialCost khong co phase
...
async function getQuarterData(quarter, year, department) {
...
class SettlementService {
  ...
  async getMonth(query) {
    const { productionScope, fromMonth, toMonth, phase, department } = query;

    if (!fromMonth || !toMonth) {
      throw { status: 400, message: "Tham số fromMonth và toMonth là bắt buộc." };
    }

    if (!department) {
      throw { status: 400, message: "Tham số phân xưởng là bắt buộc." };
    }
    ...
    return { data, mode: "byScope" };
  }

  /**
   * GET /getQuarter - Du lieu quyet toan theo quy
   */
  async getQuarter(query) {
    const { quarter, year, department } = query;

    if (!quarter || !year) {
      throw { status: 400, message: "Tham số quý và năm là bắt buộc." };
    }
   ...
   */
  async getExcel(bodyData) {
    const { fromMonth, toMonth, department } = bodyData;
    ...
    topRowLabels.forEach((label, rowIndex) => {
      ...
      setCell(dataRow, 1, rowSTT, { font: { bold: true } });
      mergeAndStyle(dataRow, 2, dataRow, 8, label, {
        font: { bold: true },
      ...
      blockCols.forEach((bkCol) => {
        if (bkCol.isSummary) {
          ...
          for (let offset = 0; offset < 6; offset++) {
            setCell(dataRow, bkCol.start + offset, "");
          }
          setCell(
            dataRow,
            bkCol.start + 2,
    ...
    // --- Row Groups (Assignments and Materials) ---
    rowGroups.forEach((assignment) => {
      // 1. Assignment Code Row
      setCell(dataRow, 1, sttCounter++, { font: { bold: true } });
      setCell(dataRow, 2, "", { font: { bold: true } });
      setCell(dataRow, 3, "", { font: { bold: true } });
      setCell(
        dataRow,
        4,
      ...
      setCell(dataRow, 5, assignment.assignmentCode?.code || "", {
        font: { bold: true },
      });
      setCell(
        dataRow,
      ...
      setCell(dataRow, 7, assignment.assignmentCode?.uom?.name || "", {
        font: { bold: true },
      });
      setCell(
        dataRow,
      ...
      blockCols.forEach((bkCol) => {
        if (bkCol.isSummary) {
          ...
          setCell(dataRow, bkCol.start + 1, "");
          setCell(dataRow, bkCol.start + 2, "");
          setCell(
            dataRow,
            bkCol.start + 3,
            assignment.assignmentCode && blockData
          ...
          setCell(dataRow, bkCol.start + 5, "");
          setCell(dataRow, bkCol.start + 6, "");
          setCell(
            dataRow,
            bkCol.start + 7,
            assignment.assignmentCode && blockData
          ...
          setCell(dataRow, bkCol.start + 8, "");
          setCell(dataRow, bkCol.start + 9, "");
          setCell(
            dataRow,
            bkCol.start + 10,
            assignment.assignmentCode && blockData
      ...
      for (let i = 0; i < assignment.maxRows; i++) {
        ...
        setCell(dataRow, 1, "");
        setCell(dataRow, 2, materialMeta.material?.code || "");
        setCell(
          dataRow,
          3,
          isDuplicate
        ...
        setCell(dataRow, 4, "");
        setCell(dataRow, 5, "");
        setCell(dataRow, 6, materialMeta.material?.name || "", {
          alignment: { horizontal: "left" },
        });
        setCell(dataRow, 7, materialMeta.material?.uom?.name || "");
        setCell(
          dataRow,
          8,
        ...
        blockCols.forEach((bkCol) => {
          if (bkCol.isSummary) {
            ...
            setCell(dataRow, bkCol.start, mu ? numOrEmpty(mu.quantity) : "");
            setCell(dataRow, bkCol.start + 1, "");
            setCell(dataRow, bkCol.start + 2, "");
            setCell(dataRow, bkCol.start + 3, mu ? numOrEmpty(mu.cost) : "");
            ...
            setCell(dataRow, bkCol.start + 5, "");
            setCell(dataRow, bkCol.start + 6, "");
            setCell(dataRow, bkCol.start + 7, mu ? numOrEmpty(mu.cost) : "");
            ...
            setCell(dataRow, bkCol.start + 11, "");
            setCell(dataRow, bkCol.start + 12, "");
          }
        });
    ...
    // Bottom Summary Row
    mergeAndStyle(dataRow, 1, dataRow, 8, "Tổng chi phí", {
      font: { bold: true },
    ...
    blockCols.forEach((bkCol) => {
      if (bkCol.isSummary) {
        ...
        setCell(dataRow, bkCol.start, "", { font: { bold: true } });
        setCell(dataRow, bkCol.start + 1, numOrEmpty(totalPlanSum), {
          font: { bold: true },
        });
        setCell(dataRow, bkCol.start + 2, "", { font: { bold: true } });
        setCell(dataRow, bkCol.start + 3, numOrEmpty(totalUsedSum), {
          font: { bold: true },
        });
        setCell(dataRow, bkCol.start + 4, "", { font: { bold: true } });
        setCell(dataRow, bkCol.start + 5, numOrEmpty(totalVarianceSum), {
          font: { bold: true },
        });
      } else if (bkCol.isOther) {
        const blockId = `${bkCol.month}_${bkCol.phaseId}`;
        ...
        setCell(dataRow, bkCol.start + 1, "");
        setCell(dataRow, bkCol.start + 2, "");
        setCell(dataRow, bkCol.start + 3, numOrEmpty(totalPlan), {
          font: { bold: true },
        });
        ...
        setCell(dataRow, bkCol.start + 5, "");
        setCell(dataRow, bkCol.start + 6, "");
        setCell(dataRow, bkCol.start + 7, numOrEmpty(totalUsed), {
          font: { bold: true },
        });

        setCell(dataRow, bkCol.start + 8, "");
        setCell(dataRow, bkCol.start + 9, numOrEmpty(totalVariance), {
          font: { bold: true },
        });
      } else {
        const blockId = `${bkCol.month}_${bkCol.phaseId}`;
        ...
        setCell(dataRow, bkCol.start + 3, "");
        setCell(dataRow, bkCol.start + 4, "");
        setCell(dataRow, bkCol.start + 5, "");
        setCell(dataRow, bkCol.start + 6, numOrEmpty(totalPlan), {
          font: { bold: true },
        });

        setCell(dataRow, bkCol.start + 7, "");
        setCell(dataRow, bkCol.start + 8, "");
        setCell(dataRow, bkCol.start + 9, "");
        setCell(dataRow, bkCol.start + 10, numOrEmpty(totalUsed), {
          font: { bold: true },
        });

        setCell(dataRow, bkCol.start + 11, "");
        setCell(dataRow, bkCol.start + 12, numOrEmpty(totalVariance), {
          font: { bold: true },
        });
      }
    });
   ...
   */
  async getQuarterExcel(bodyData) {
    const { quarter, year, department } = bodyData || {};
    ...
    const result = await Promise.all(
      months.map(async (m) => {
        ...
        return {
          ...
          grossLoss,
          grossSavings,
          variance: grossLoss + grossSavings,
        };
      }),
    );
   ...
   */
  async updateMaterialAssignmentCode(body) {
    ...
    if (newPrice != null) {
      ...
      if (!doc || !doc.materials || doc.materials.length === 0) {
        throw { status: 404, message: "Không tìm thấy vật tư cần cập nhật." };
      }
    ...
    return "Cập nhật mã giao khoán, đơn giá và chi phí thành công.";
  }

  /**
   * PATCH /updateMaterialQuantity - Cap nhat so luong vat tu
   */
  async updateMaterialQuantity(body) {
    const { materialCostId, materialItemId, newQuantity } = body;
    ...
    return "Cập nhật số lượng thực hiện thành công.";
  }
}
