...
class ThicknessController extends BaseController {
  ...
  export = async (req, res, next) => {
    try {
      ...
      const columns = [
        { header: 'Độ dày vỉa than', key: 'name', width: 30 },
        { header: '_id', key: '_id', width: 20 },
      ];
      const { workbook, worksheet } = createWorkbook(columns, data, 'thickness');
      ...
      sendExcelResponse(res, buffer, 'thickness.xlsx');
    } catch (error) { next(error); }
  };
}

module.exports = ThicknessController;
