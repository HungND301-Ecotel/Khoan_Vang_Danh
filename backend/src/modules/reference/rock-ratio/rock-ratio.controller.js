...
class RockRatioController extends BaseController {
  ...
  export = async (req, res, next) => {
    try {
      ...
      const columns = [
        { header: 'Tỷ lệ đá', key: 'name', width: 30 },
        { header: '_id', key: '_id', width: 20 },
      ];
      const { workbook, worksheet } = createWorkbook(columns, data, 'rock-ratio');
      ...
      sendExcelResponse(res, buffer, 'rock-ratio.xlsx');
    } catch (error) { next(error); }
  };
}

module.exports = RockRatioController;
