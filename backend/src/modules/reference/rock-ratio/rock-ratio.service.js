...
const RockRatio = require('./rock-ratio.model');
const ConflictError = require('../../../shared/exceptions/ConflictError');

class RockRatioService extends BaseService {
  constructor() {
    super(RockRatio);
  }
  ...
  async create(data) {
    ...
    if (exists) {
      throw new ConflictError(`Tỷ lệ đá '${name.trim()}' đã tồn tại`);
    }
  ...
  async update(id, data) {
    ...
    if (!doc) throw new NotFoundError('Không tìm thấy tỷ lệ đá');
    return doc;
  }
  ...
  async importData(data) {
    ...
    for (const item of data) {
      ...
      if (_id && cleanName) {
        if (existedId && existedId !== _id) { invalidRows.push({ item, error: `Tỷ lệ đá đã tồn tại: ${cleanName}` }); continue; }
        operations.push({ updateOne: { filter: { _id }, update: { $set: { name: cleanName, ...updateData } } } });
        continue;
      }
      if (!_id && cleanName) {
        if (existedId) { invalidRows.push({ item, error: `Tỷ lệ đá đã tồn tại: ${cleanName}` }); continue; }
...
module.exports = RockRatioService;
