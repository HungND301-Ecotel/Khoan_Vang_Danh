...
const Hardness = require('./hardness.model');
const ConflictError = require('../../../shared/exceptions/ConflictError');

class HardnessService extends BaseService {
  constructor() {
    super(Hardness);
  }
  ...
  async create(data) {
    ...
    if (exists) {
      throw new ConflictError(`Độ cứng đá '${name.trim()}' đã tồn tại`);
    }
  ...
  async update(id, data) {
    ...
    if (!doc) throw new NotFoundError('Không tìm thấy độ cứng đá');
    return doc;
  }
  ...
  async importData(data) {
    ...
    for (const item of data) {
      ...
      if (_id && cleanName) {
        if (existedId && existedId !== _id) { invalidRows.push({ item, error: `Độ cứng đá đã tồn tại: ${cleanName}` }); continue; }
        operations.push({ updateOne: { filter: { _id }, update: { $set: { name: cleanName, ...updateData } } } });
        continue;
      }
      if (!_id && cleanName) {
        if (existedId) { invalidRows.push({ item, error: `Độ cứng đá đã tồn tại: ${cleanName}` }); continue; }
...
module.exports = HardnessService;
