...
class StepController extends BaseController {
  ...
  export = async (req, res, next) => {
    try {
      ...
      const columns = [
        { header: 'Bước chống giữ', key: 'name', width: 30 },
        { header: '_id', key: '_id', width: 20 },
      ];
      const { workbook, worksheet } = createWorkbook(columns, data, 'step');
      ...
      sendExcelResponse(res, buffer, 'step.xlsx');
    } catch (error) { next(error); }
  };
}
