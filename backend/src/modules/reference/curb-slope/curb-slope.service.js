...
const CurbSlope = require('./curb-slope.model');
const ConflictError = require('../../../shared/exceptions/ConflictError');

class CurbSlopeService extends BaseService {
  constructor() {
    super(CurbSlope);
  }
  ...
  async create(data) {
    ...
    if (exists) {
      throw new ConflictError(`Độ dốc vỉa than '${name.trim()}' đã tồn tại`);
    }
  ...
  async update(id, data) {
    ...
    if (!doc) throw new NotFoundError('Không tìm thấy độ dốc vỉa than');
    return doc;
  }
  ...
  async importData(data) {
    ...
    for (const item of data) {
      ...
      if (_id && cleanName) {
        if (existedId && existedId !== _id) { invalidRows.push({ item, error: `Độ dốc vỉa than đã tồn tại: ${cleanName}` }); continue; }
        operations.push({ updateOne: { filter: { _id }, update: { $set: { name: cleanName, ...updateData } } } });
        continue;
      }
      if (!_id && cleanName) {
        if (existedId) { invalidRows.push({ item, error: `Độ dốc vỉa than đã tồn tại: ${cleanName}` }); continue; }
...
module.exports = CurbSlopeService;
