...
const Length = require('./length.model');
const ConflictError = require('../../../shared/exceptions/ConflictError');

class LengthService extends BaseService {
  constructor() {
    super(Length);
  }
  ...
  async create(data) {
    ...
    if (exists) {
      throw new ConflictError(`Chiều dài '${name.trim()}' đã tồn tại`);
    }
  ...
  async update(id, data) {
    ...
    if (!doc) throw new NotFoundError('Không tìm thấy chiều dài');
    return doc;
  }
  ...
  async importData(data) {
    ...
    for (const item of data) {
      ...
      if (_id && cleanName) {
        if (existedId && existedId !== _id) { invalidRows.push({ item, error: `Chiều dài đã tồn tại: ${cleanName}` }); continue; }
        operations.push({ updateOne: { filter: { _id }, update: { $set: { name: cleanName, ...updateData } } } });
        continue;
      }
      if (!_id && cleanName) {
        if (existedId) { invalidRows.push({ item, error: `Chiều dài đã tồn tại: ${cleanName}` }); continue; }
