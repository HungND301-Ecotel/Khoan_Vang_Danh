...
class LengthController extends BaseController {
  ...
  export = async (req, res, next) => {
    try {
      ...
      const columns = [
        { header: 'Chiều dài', key: 'name', width: 30 },
        { header: '_id', key: '_id', width: 20 },
      ];
      const { workbook, worksheet } = createWorkbook(columns, data, 'length');
      ...
      sendExcelResponse(res, buffer, 'length.xlsx');
    } catch (error) { next(error); }
  };
}
