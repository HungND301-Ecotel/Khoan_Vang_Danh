...
class UnitService extends BaseService {
  ...
  async create(data) {
    ...
    if (exists) {
      throw new ConflictError(`Đơn vị tính '${name.trim()}' đã tồn tại`);
    }
  ...
  async update(id, data) {
    ...
    if (!doc) throw new NotFoundError('Không tìm thấy đơn vị tính');
    return doc;
  }
  ...
  async importData(data) {
    const operations = [];
    const invalidRows = [];
    const units = await this.model.find({}, { name: 1 }).lean();
    const nameMap = new Map(units.map(u => [u.name.toLowerCase(), String(u._id)]));
    ...
    for (const item of data) {
      ...
      if (_id && cleanName) {
        if (existedId && existedId !== _id) { invalidRows.push({ item, error: `Đơn vị tính đã tồn tại: ${cleanName}` }); continue; }
        operations.push({ updateOne: { filter: { _id }, update: { $set: { name: cleanName, ...updateData } } } });
        continue;
      }
      if (!_id && cleanName) {
        if (existedId) { invalidRows.push({ item, error: `Đơn vị tính đã tồn tại: ${cleanName}` }); continue; }
