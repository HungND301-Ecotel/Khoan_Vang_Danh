...
class ExcavationTechController extends BaseController {
  ...
  export = async (req, res, next) => {
    try {
      ...
      const columns = [
        { header: 'Công nghệ đào lò', key: 'name', width: 30 },
        { header: '_id', key: '_id', width: 20 },
      ];
      const { workbook, worksheet } = createWorkbook(columns, data, 'excavation-tech');
      ...
      sendExcelResponse(res, buffer, 'excavation-tech.xlsx');
    } catch (error) { next(error); }
  };
}

module.exports = ExcavationTechController;
