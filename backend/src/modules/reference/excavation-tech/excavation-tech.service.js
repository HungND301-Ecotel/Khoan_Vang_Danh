...
const ExcavationTech = require('./excavation-tech.model');
const ConflictError = require('../../../shared/exceptions/ConflictError');

class ExcavationTechService extends BaseService {
  constructor() {
    super(ExcavationTech);
  }
  ...
  async create(data) {
    ...
    if (exists) {
      throw new ConflictError(`Công nghệ đào lò '${name.trim()}' đã tồn tại`);
    }
  ...
  async update(id, data) {
    ...
    if (!doc) throw new NotFoundError('Không tìm thấy công nghệ đào lò');
    return doc;
  }
  ...
  async importData(data) {
    ...
    for (const item of data) {
      ...
      if (_id && cleanName) {
        if (existedId && existedId !== _id) { invalidRows.push({ item, error: `Công nghệ đào lò đã tồn tại: ${cleanName}` }); continue; }
        operations.push({ updateOne: { filter: { _id }, update: { $set: { name: cleanName, ...updateData } } } });
        continue;
      }
      if (!_id && cleanName) {
        if (existedId) { invalidRows.push({ item, error: `Công nghệ đào lò đã tồn tại: ${cleanName}` }); continue; }
...
module.exports = ExcavationTechService;
