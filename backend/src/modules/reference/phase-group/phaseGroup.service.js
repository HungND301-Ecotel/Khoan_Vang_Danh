...
const PhaseGroup = require('./phaseGroup.model');
...
class PhaseGroupService extends BaseService {
  constructor() {
    super(PhaseGroup);
  }
  ...
  async create(data) {
    ...
    const { isDuplicate, collectionName } = await checkUniqueCode(code, null, 'PhaseGroup');
    if (isDuplicate) {
    ...
    if (existsName) {
      throw new ConflictError(`Nhóm công đoạn '${name.trim()}' đã tồn tại`);
    }
  ...
  async update(id, data) {
    ...
    // Kiểm tra code trùng toàn hệ thống
    const { isDuplicate, collectionName } = await checkUniqueCode(code, id, 'PhaseGroup');
    if (isDuplicate) {
      throw new ConflictError(`Mã '${code.trim()}' đã tồn tại trong hệ thống`);
    }
    ...
    if (!doc) {
      throw new NotFoundError('Không tìm thấy nhóm công đoạn');
    }
  ...
  async importData(data) {
    ...
    for (const item of data) {
      ...
      if (!cleanCode || !cleanName) {
        invalidRows.push({ item, error: 'Mã và Tên nhóm công đoạn là bắt buộc' });
        continue;
      }
...
module.exports = PhaseGroupService;
