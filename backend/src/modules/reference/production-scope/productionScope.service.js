...
const ProductionScope = require('./productionScope.model');
...
class ProductionScopeService extends BaseService {
  constructor() {
    super(ProductionScope);
  }
  ...
  async create(data) {
    ...
    const { isDuplicate, collectionName } = await checkUniqueCode(code, null, 'ProductionScope');
    if (isDuplicate) {
    ...
    if (existsName) {
      throw new ConflictError(`Phạm vi sản xuất '${name.trim()}' đã tồn tại`);
    }
  ...
  async update(id, data) {
    ...
    // Kiểm tra code trùng toàn hệ thống
    const { isDuplicate, collectionName } = await checkUniqueCode(code, id, 'ProductionScope');
    if (isDuplicate) {
      throw new ConflictError(`Mã '${code.trim()}' đã tồn tại trong hệ thống`);
    }
    ...
    if (!doc) {
      throw new NotFoundError('Không tìm thấy phạm vi sản xuất');
    }
  ...
  async importData(data) {
    ...
    for (const item of data) {
      ...
      if (!cleanCode || !cleanName) {
        invalidRows.push({ item, error: 'Mã và Tên phạm vi sản xuất là bắt buộc' });
        continue;
      }
      ...
      if (cleanCode) {
        const checkGlobal = await checkUniqueCode(cleanCode, _id, 'ProductionScope');
...
module.exports = ProductionScopeService;
