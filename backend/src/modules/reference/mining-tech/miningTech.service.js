...
const MiningTech = require('./miningTech.model');
...
class MiningTechService extends BaseService {
  constructor() {
    super(MiningTech);
  }
  ...
  async create(data) {
    ...
    const { isDuplicate, collectionName } = await checkUniqueCode(code, null, 'MiningTech');
    if (isDuplicate) {
    ...
    if (existsName) {
      throw new ConflictError(`Công nghệ khai thác '${name.trim()}' đã tồn tại`);
    }
  ...
  async update(id, data) {
    ...
    // Kiểm tra code trùng toàn hệ thống
    const { isDuplicate, collectionName } = await checkUniqueCode(code, id, 'MiningTech');
    if (isDuplicate) {
      throw new ConflictError(`Mã '${code.trim()}' đã tồn tại trong hệ thống`);
    }
    ...
    if (!doc) {
      throw new NotFoundError('Không tìm thấy công nghệ khai thác');
    }
  ...
  async importData(data) {
    ...
    for (const item of data) {
      ...
      if (!cleanCode || !cleanName) {
        invalidRows.push({ item, error: 'Mã và Tên công nghệ khai thác là bắt buộc' });
        continue;
      }
...
module.exports = MiningTechService;
