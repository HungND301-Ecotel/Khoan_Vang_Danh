...
exports.import = async (req, res) => {
  try {
    ...
    const lengths = await Length.find({}, { name: 1 }).lean();
    const nameMap = new Map(
      lengths.map(u => [u.name.toLowerCase(), String(u._id)])
    );
    ...
    for (const item of dataImport) {
      ...
      if (_id && cleanName) {
        if (existedId && existedId !== _id) {
          invalidRows.push({
            item,
            error: `Độ dài lò đã tồn tại: ${cleanName}`,
          });
          continue;
        }
    ...
    if (operations.length > 0) {
      bulkResult = await Length.bulkWrite(operations);
    }

    res.status(200).json({
      status: "success",
      message: "Import độ dài lò hoàn tất",
      summary: {
...
exports.export = async (req, res) => {
  try {
    ...
    const worksheet = workbook.addWorksheet("length");
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=" + `length.xlsx`
    );
    res.send(buffer);
  } catch (err) {
    res
...
exports.update = async (req, res) => {
  try {
    const updateData = await Length.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
    if (!updateData) {
...
exports.delete = async (req, res) => {
  try {
    const deleteData = await Length.findByIdAndDelete(req.params.id);
    if (!deleteData) {
...
exports.deleteMany = async (req, res) => {
  try {
    ...
    const result = await Length.deleteMany({ _id: { $in: ids } });
...
exports.get = async (req, res) => {
  try {
    ...
    const modelQuery = Length.find(query);
