...
exports.create = async (req, res) => {
  try {
    const { name } = req.body;
    const exitData = await Hardness.countDocuments({ name: name })
    if (exitData > 0) {
      return res.status(409).json({ status: 'error', message: `Độ cứng than/ đá '${name}' đã tồn tại` })
    }
...
exports.import = async (req, res) => {
  try {
    ...
    const hardness = await Hardness.find({}, { name: 1 }).lean();
    const nameMap = new Map(
      hardness.map(u => [u.name.toLowerCase(), String(u._id)])
    );
    ...
    for (const item of dataImport) {
      ...
      if (_id && cleanName) {
        if (existedId && existedId !== _id) {
          invalidRows.push({
            item,
            error: `Độ cứng đã tồn tại: ${cleanName}`,
          });
          continue;
        }
    ...
    if (operations.length > 0) {
      bulkResult = await Hardness.bulkWrite(operations);
    }

    res.status(200).json({
      status: "success",
      message: "Import độ cứng hoàn tất",
      summary: {
...
exports.delete = async (req, res) => {
  try {
    ...
    const result = await Hardness.deleteMany({ _id: { $in: ids } });
