...
exports.import = async (req, res) => {
  try {
    ...
    const steps = await Step.find({}, { name: 1 }).lean();
    const nameMap = new Map(
      steps.map(u => [u.name.toLowerCase(), String(u._id)])
    );
    ...
    for (const item of dataImport) {
      ...
      if (_id && cleanName) {
        if (existedId && existedId !== _id) {
          invalidRows.push({
            item,
            error: `Chống đã tồn tại: ${cleanName}`,
          });
          continue;
        }
    ...
    if (operations.length > 0) {
      bulkResult = await Step.bulkWrite(operations);
    }

    res.status(200).json({
      status: "success",
      message: "Import chống hoàn tất",
      summary: {
...
exports.deleteMany = async (req, res) => {
  try {
    ...
    const result = await Step.deleteMany({ _id: { $in: ids } });
