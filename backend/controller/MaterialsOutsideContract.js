...
exports.create = async (req, res) => {
  try {
    ...
    const { code, name, assignmentCode, uom, quantity, priceHistory } =
      req.body;
    const newMaterialAssignment = new MaterialsOutsideContract({
      code,
      name,
      assignmentCode,
      uom,
...
exports.update = async (req, res) => {
  try {
    ...
    const updateData = await MaterialsOutsideContract.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
exports.delete = async (req, res) => {
  try {
    // SỬA: MaterialsOutsideContract thay vì MaterialAssignment
    const deleteData = await MaterialsOutsideContract.findByIdAndDelete(
      req.params.id
    );
    if (!deleteData) {
...
exports.getAll = async (req, res) => {
  try {
    ...
    const assignmentIds = [
      ...new Set(
        materials.map((m) => m.assignmentCode?._id?.toString()).filter(Boolean)
      ),
    ];
