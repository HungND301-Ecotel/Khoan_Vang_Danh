...
exports.create = async (req, res) => {
  try {
    ...
    const newMaterialCostUsed = new MaterialCostUsed({
      code,
      productionScope,
      materials,
    });
...
exports.update = async (req, res) => {
  try {
    const updateData = await MaterialCostUsed.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
  ...
  try {
    ...
    const processedData = data.map((doc) => {
      ...
      docObj.materials = docObj.materials.map((mat) => {
        ...
        // nếu material null thì trả mặc định
        if (!material) {
          return {
            ...mat,
            cost: 0,
        ...
        if (Array.isArray(material.priceHistory)) {
          const matched = material.priceHistory.find(
            (priceItem) =>
