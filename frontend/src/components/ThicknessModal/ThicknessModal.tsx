...
export default function ThicknessModal({
  open,
  setOpen,
  handleSubmit,
  selectedThickness,
    ...
    >
      ...
      <DialogTitle sx={{ p: 0, mt: "16px" }}>
        <Breadcrumbs aria-label="breadcrumb" sx={{ fontSize: "14px" }}>
          ...
          <Typography>Độ dày vỉa (Mv)</Typography>
        </Breadcrumbs>
        <Divider
          style={{
            margin: "10px 0",
        ...
        <Typography sx={{ fontSize: "24px", color: "#2B4A82" }}>
          {selectedThickness ? "Chỉnh sửa Độ dày vỉa (Mv)" : "Tạo mới Độ dày vỉa (Mv)"}
        </Typography>
      </DialogTitle>
      ...
      <DialogContent sx={{ p: 0, mt: 3 }}>
        <Box component="form" onSubmit={formik.handleSubmit} sx={{ display: "flex", justifyContent: "center" }}>
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2, width: "700px" }}>
            <Box>
              <Typography sx={{ fontWeight: 500, fontSize: "14px", mb: 1 }}>Độ dày vỉa</Typography>
              <TextField
                fullWidth
                id="name"
                name="name"
