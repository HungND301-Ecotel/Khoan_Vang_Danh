...
import { AssignmentCodeOutputType, MaterialAssignmentInputType, Materials, UnitType } from '../../types'
...
export default function MaterialAssignmentModal({ open, setOpen, handleSubmit, selectedMaterialAssignment }: { open: boolean; setOpen: Dispatch<SetStateAction<boolean>>; handleSubmit: (values: Partial<MaterialAssignmentInputType>) => void; selectedMaterialAssignment: Materials | null }) {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      uom: selectedMaterialAssignment ? selectedMaterialAssignment.uom?._id : '',
      quantity: selectedMaterialAssignment ? selectedMaterialAssignment.quantity : '',
      assignmentCode: selectedMaterialAssignment ? selectedMaterialAssignment.assignmentCode?._id : '',
                    ...
                    {formik.values.priceHistory.map((item, index) => (
                      <Box key={index} sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                        <TextField
                          ...
                          onChange={(e) => formik.setFieldValue(`priceHistory[${index}].startDate`, e.target.value)}
                        ...
                        <TextField
                          ...
                          onChange={(e) => formik.setFieldValue(`priceHistory[${index}].endDate`, e.target.value)}
                          InputLabelProps={{ shrink: true }}
                        />
                        <TextField
                          fullWidth
                          ...
                          onChange={(e) => formik.setFieldValue(`priceHistory[${index}].price`, e.target.value)}
                        />
