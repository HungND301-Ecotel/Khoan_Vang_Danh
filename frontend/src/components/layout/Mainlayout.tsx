...
const MainLayout: React.FC<MainLayoutProps> = ({ children }) => {
    ...
    return (
        <Box sx={{ display: "flex", minHeight: "100vh", bgcolor: "background.default" }}>
            ...
            >
                <Toolbar sx={{ gap: 4, height: 100, display: 'flex', alignItems: 'center' }}>
                    {/* Logo + Company */}
                    <img src="/logo.png" style={{ width: 150, height: 100 }} />

                    {/* NAV BUTTONS */}
                    <Box sx={{ flex: 1, display: 'flex', gap: 1.25, ml: 3, justifyContent: 'space-between', alignItems: 'center' }}>
                        <Button startIcon={<DashboardIcon sx={{ color: 'blue' }} />} sx={{ color: 'black', }} onClick={() => navigate("/")}>
                            TỔNG QUAN
                        </Button>
                        <Button startIcon={<ListAlt sx={{ color: 'blue' }} />} sx={{ color: 'black', }} onClick={(e) => setMenuDanhMucEl(e.currentTarget)}>
                            DANH MỤC
                        </Button>
                        <Button startIcon={<Calculate sx={{ color: 'blue' }} />} sx={{ color: 'black', }} onClick={(e) => setMenuDonGiaEl(e.currentTarget)}>
                            ĐƠN GIÁ VÀ ĐỊNH MỨC
                        </Button>
                        <Button startIcon={<Equalizer sx={{ color: 'blue' }} />} sx={{ color: 'black', }} onClick={(e) => setMenuThongKeEl(e.currentTarget)}>
                            THỐNG KÊ VẬN HÀNH
                        </Button>
                    </Box>
                    ...
                    <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                        ...
                        <Box display='flex' alignItems={'center'} gap={2} sx={{ '&:hover': { cursor: 'pointer' } }} borderLeft={'1px solid grey'} onClick={(e) => setMenuSettingsEl(e.currentTarget)}>
            ...
            >
                ...
                {phases
                    .filter((i: PhaseOutputType) => i.phaseGroup?.name?.toLowerCase() === "khấu than")
