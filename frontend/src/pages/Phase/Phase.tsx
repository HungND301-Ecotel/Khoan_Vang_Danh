...
export default function Phase() {
  ...
  const handleDeleteMultiple = () => {
    ...
    showConfirmAlert(`Bạn có muốn xóa ${selectedPhases.length} bản ghi đã chọn?`).then((result) => {
      if (result.isConfirmed) {
        ...
        Promise.all(deletePromises)
          .then(() => {
            queryClient.invalidateQueries({ queryKey: ["phases"] });
            setSelectedPhases([]);
            showSuccessAlert(`Đã xóa ${selectedPhases.length} bản ghi thành công`);
          })
          .catch((error) => {
  ...
  const columns: TableProps<PhaseOutputType>['columns'] = [
    ...
    {
      title: <Typography sx={{ fontWeight: 'bold' }}>Nhóm công đoạn</Typography>,
      dataIndex: 'phaseGroup',
      key: 'phaseGroup',
      render: (_, record) => (
        <Typography sx={{ fontWeight: 'bold' }}>{record.phaseGroup?.name}</Typography>
      ),
      sorter: (a, b) =>
        (a.phaseGroup?.name ?? '').localeCompare(b.phaseGroup?.name ?? '', 'vi', { sensitivity: 'base' }),
    },
    {
  ...
  const filteredPhases = phases.filter((item: PhaseOutputType) => 
