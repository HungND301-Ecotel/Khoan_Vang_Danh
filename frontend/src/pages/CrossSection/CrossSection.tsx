...
export default function CrossSection() {
  ...
  const { data: crosssections = [], isLoading, isFetching } = useQuery({
    queryKey: ["crosssections"],
    queryFn: async () => {
      try {
        const response = await api.get("/crosssections");
  ...
  const filteredCrossSections = useMemo(() => {
    ...
    return crosssections.filter((item: CrossSectionOutputType) => {
      const name = item.name?.toLowerCase() || "";
      const uomName = item.uom?.name?.toLowerCase() || "";
      
      return name.includes(searchTerm) || uomName.includes(searchTerm);
    });
  }, [crosssections, searchValue]);
            ...
            <Box sx={{ mb: 2, p: 1, backgroundColor: "#f0f7ff", borderRadius: 1 }}>
              <Typography variant="body2" color="primary">
                {isFiltering ? (
                  ...
                  <>
                    Tìm thấy {filteredCrossSections.length} kết quả cho "{searchValue}"
                    {filteredCrossSections.length > 0 && (
                      <Button 
                        size="small" 
                        onClick={handleClearSearch}
                        sx={{ ml: 2 }}
