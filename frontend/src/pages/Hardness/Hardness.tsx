...
export default function Hardness() {
  ...
  const handleDeleteMultiple = () => {
    ...
    showConfirmAlert(`Bạn có muốn xóa ${selectedHardnesses.length} bản ghi đã chọn?`).then((result) => {
      if (result.isConfirmed) {
        ...
        Promise.all(deletePromises)
          .then(() => {
            ...
            showSuccessAlert(`Đã xóa ${selectedHardnesses.length} bản ghi thành công`);
          })
          .catch((error) => {
  ...
  const columns: TableProps<HardnessType>['columns'] = [
    ...
    {
      title: <Typography sx={{ fontWeight: 'bold' }}>Độ cứng của than/ đá (f)</Typography>,
      dataIndex: 'name',
      key: 'name',
      render: (_, record) => (
