...
import { Typography, Box, Paper, TextField, CircularProgress } from '@mui/material';
...
export default function CostProfitChart() {
    ...
    const { data: dashboardData, isLoading } = useQuery({
        ...
        queryFn: () => api.get(`/contractsettlements/getDashboardData?year=${selectedYear}`).then(res => res.data.data),
    ...
    const chartData = dashboardData 
        ? dashboardData.map((item: any) => {
            ...
            const monthNum = monthArr.length > 1 ? parseInt(monthArr[1], 10) : item.month;
            return {
