...
import { Box, Typography, Paper, Stack, alpha, CircularProgress } from '@mui/material';
...
export default function CostCard() {
    const { data: dashboardData, isLoading } = useQuery({
        ...
        queryFn: () => api.get('/contractsettlements/getDashboardData').then(res => res.data.data),
    });
    ...
    const latestData = dashboardData && dashboardData.length > 0 
        ? dashboardData[dashboardData.length - 1] 
        : { totalUsed: 0, variance: 0, month: '' };
    
    // Tạo nhãn tháng cho biểu đồ (T11, T12, T1...)
    const monthLabels = dashboardData ? dashboardData.map((item: any) => {
        const [, m] = item.month.split('-');
        return `T${parseInt(m)}`;
    }) : [];

    const dailyData = dashboardData ? dashboardData.map((item: any) => item.totalUsed) : [];
    ...
    const percent = budget > 0 ? (Math.abs(latestData.variance) / budget) * 100 : 0;
    ...
    return (
        ...
        >
            ...
            <Stack spacing={3} sx={{ position: 'relative', zIndex: 1, height: '100%', justifyContent: 'space-between' }}>
                ...
                <Box display="flex" justifyContent="space-between" alignItems="flex-start">
                    <Stack spacing={0.5}>
                        ...
                        <Typography variant="body2" sx={{ fontWeight: 700, color: chartColor, fontSize: '1rem' }}>
                            {currentMonthYear}
                        </Typography>
                    </Stack>
