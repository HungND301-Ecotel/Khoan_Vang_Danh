...
const DepartmentService = {
    getAll: async (params?: Record<string, any>): Promise<DepartmentType[]> => {
        const res = await api.get('/departments', { params });
        return res.data.data
    },
    create: async (data: Partial<DepartmentType>): Promise<any> => {
        const res = await api.post('/departments', data);
        return res.data
    },
    update: async (data: Partial<DepartmentType>): Promise<any> => {
